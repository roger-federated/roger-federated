...
The *client* runs the tools, and their results reach us as `role: "tool"` messages (chat) or
`function_call_output` items (responses) inside the next request, so the verifiable part of the reward
— a nonzero exit code, an error string, a rejected command — is recovered by reading that history
rather than by running anything. Stays in the runtime package and torch-free: it runs on every captured
request, long before a training round is due.
"""
import json, re

# Sign and relative size are what matter: REINFORCE++ z-normalises the advantages, so the absolute
# scale is forgiving.
W_CMD_REJ = 0.3     # user rejected a command
W_EXIT = 0.1        # nonzero exit code
W_ERROR = 0.1       # result reads as an error

# The result text is in whatever format the client's tools produce, so the exit-code pattern takes the
# common phrasings: `exit 1`, `Exit code: 1`, `exit status 2`, `exited with code 1`, `[exit code 1]`.
...
def step_rewards(items) -> dict[str, float]:
    ...
    A step is one generation turn as the wire shows it: a run of assistant-side items (one chat
